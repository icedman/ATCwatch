...
void get_read_touch() {
    ...
    if (touch_data.xpos == 255 && touch_data.ypos == 255) {
        ...
        if (touch_data.last_xpos != touch_data.xpos &&
            touch_data.last_ypos != touch_data.ypos) {
            touch_data.last_time = millis();
            didMove = true;
            touch_data.last_gesture = touch_data.gesture;
        }
        touch_data.last_xpos = touch_data.xpos;
        touch_data.last_ypos = touch_data.ypos;
    }

    touch_data.down = touch_data.down || didMove;
}

touch_data_struct get_touch() {
    long elapsed = millis() - touch_data.last_time;
    bool wasDown = touch_data.down;
    touch_data.down = touch_data.down && touch_data.last_time > 0 && elapsed < 350;

    if (touch_data.down == true && touch_data.start_time == 0) {
        touch_data.start_time = touch_data.last_time;
        touch_data.start_xpos = touch_data.xpos;
        touch_data.start_ypos = touch_data.ypos;
    }
    ...
    // if (!_touch_data.down && wasDown && elapsed > 100) {
    //     if (_touch_data.last_gesture > 0 && _touch_data.last_gesture < 5) {
    //         _touch_data.gesture = _touch_data.last_gesture;
    //         _touch_data.down = true;
    //         // check_menu(_touch_data); << check events
    //         // _touch_data.down = false;
    //     }
    // }

    touch_data.gesture = 0;
    return _touch_data;
}
